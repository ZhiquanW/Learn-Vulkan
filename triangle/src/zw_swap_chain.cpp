...
VkResult ZWSwapChain::acquireNextImage(uint32_t *imageIndex) {
    ...
    VkResult result = vkAcquireNextImageKHR(
        ...
        imageAvailableSemaphores[currentFrame],  // must be a not
                                                 // signaled semaphore
...
void ZWSwapChain::createSwapChain() {
    ...
    // so the implementation is allowed to create a swap chain with
    // more. That's why we'll first query the final number of images
    // with vkGetSwapchainImagesKHR, then resize the container and
