...
void ZWPipeline::createGraphicsPipeline(
    ...
    assert(configInfo.pipelineLayout != nullptr &&
           "Cannot create graphics pipeline: no pipelineLayout "
           "provided in config info");
    ...
    if (vkCreateGraphicsPipelines(
            this->zwDevice.device(), VK_NULL_HANDLE, 1, &pipelineInfo,
...
void ZWPipeline::createShaderModule(const std::vector<char>& code,
    ...
    if (vkCreateShaderModule(zwDevice.device(), &createInfo, nullptr,
...
PipelineConfigInfo ZWPipeline::defaultPipelineConfigInfo(
    ...
    configInfo.rasterizationInfo.depthClampEnable =
        VK_FALSE;  // clamp gl_Positon.z to  [0,1]
    configInfo.rasterizationInfo.rasterizerDiscardEnable =
        VK_FALSE;  // enable if only the earlier stage needed
    configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
    configInfo.rasterizationInfo.lineWidth = 1.0f;
    configInfo.rasterizationInfo.cullMode =
        VK_CULL_MODE_NONE;  // discard tri based on winding order
